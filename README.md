...
![Security](https://img.shields.io/badge/Security-Trivy_SAST-success?style=for-the-badge)
![Observability](https://img.shields.io/badge/Observability-Prometheus_%26_Grafana-orange?style=for-the-badge)
...
* **DevSecOps & Zero-Trust:**
  * **Static Application Security Testing (SAST):** Trivy vulnerability scanning integrated directly into the pipeline to block critical CVEs from reaching production.
...
1. **Checkout & Setup:** Initializes the environment and pulls the latest source code.
2. **Secure Authentication:** Executes Docker Login utilizing encrypted GitHub Secrets to prevent credential leakage.
3. **Build & Containerize:** Compiles an optimized Docker image and prepares it for distribution.
4. **Security Gate (Trivy):** Intercepts the image to scan for vulnerabilities. The pipeline is configured to automatically fail if "Critical" or "High" risk CVEs are detected, safeguarding the production environment.
5. **Artifact Push:** Upon passing the security gate, the verified image is pushed to the container registry.
...
```bash
git clone [https://github.com/marioscloud/k8s-devsecops-observability.git](https://github.com/marioscloud/k8s-devsecops-observability.git) 
...
## 📬 Author
* **Mario Araos** | Cloud & DevOps Engineer | CKA | LFCS 
* **Focus:** Passionate about resilient infrastructure, DevSecOps, and automation.
